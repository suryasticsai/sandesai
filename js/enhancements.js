// ================================================================
// js/enhancements.js  (v9 – OTP fixed)
...
//
// What changed vs v8
//   • sendOtpEmail used mode:'no-cors' → every failure looked like success.
//     It now uses GET (readable) and shows the server's actual error.
//   • checkAvailability now works (backend supports GET), and is run
//     BEFORE the OTP is consumed so a failed check doesn't waste the code.
//   • Shared helpers remove ~300 lines of duplicated OTP / styling code.
//   • Username format validation, request timeouts, OTP autofill attrs.
//   • Delete-account reads the server response before wiping local data.
// ================================================================
(function () {
    'use strict';
    ...
    async function fetchJson(url, timeoutMs) {
        ...
        try {
            ...
            try { return JSON.parse(text); }
                ...
                // Apps Script returns an HTML page for auth / deployment problems
    ...
    // (client-side hash only deters casual tampering — not real security)
    // ────────────────────────────────────────────────────────────
    ...
    async function logRegistrationToSheet(name, username, phone) {
        ...
        try {
            // no-cors: response unreadable by design; backend validates OTP flag
    ...
    // Duplicate phone / username check (GET → readable). Fails open on network errors.
    ...
    // Send OTP via GET so we can READ the real result (the old no-cors POST
    // always reported success even when the backend failed).
    async function sendOtpEmail(phone, email) {
        try {
            ...
            if (!data.ok) {
                ...
                // Old deployment: doGet doesn't know sendOtp and returns the users list / forbidden
    ...
    // ── Shared "Send OTP" button wiring (used by boot + invite screens) ──
    ...
    function showInviteWelcomeOverlay(payload) {
        ...
        $('inviteJoinBtn').addEventListener('click', async () => {
            ...
            if (isIntendedRecipient) autoRegisterFromInvite(payload, name, phone, payload.from || payload.chat);
            else autoRegisterAsNewUser(name, phone);
        });
    ...
    function showBootRegistrationScreen() {
        ...
        card.innerHTML = `
            <div style="text-align:center;margin-bottom:28px;">
                <img src="sandesai-logo.png" alt="Sandesai"
                     style="width:88px;height:88px;border-radius:50%;margin:0 auto 14px;display:block;
            ...
            <div style="margin-top:16px;text-align:center;font-size:0.7rem;color:#5a6885;line-height:1.5;">
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            // 1) Availability FIRST — so a taken username doesn't burn the OTP
    ...
    console.log('✨ enhancements.js v9 loaded — OTP fixed (readable responses), availability check, boot gate, invites, sync, sheets, debug, memory, delete-account');
})();
